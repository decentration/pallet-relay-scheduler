...
pub use cumulus_primitives_core::relay_chain::BlockNumber;
...
pub mod pallet {
	...
	use sp_runtime::traits::{MaybeDisplay, Bounded, AtLeast32BitUnsigned, MaybeMallocSizeOf};
	...
	pub trait Config: frame_system::Config {
		...
		// Relay type
		type RelayBlockNumber: Parameter + Member + MaybeSerializeDeserialize + Debug + MaybeDisplay + AtLeast32BitUnsigned + Default + Bounded + Copy + sp_std::hash::Hash + sp_std::str::FromStr + MaybeMallocSizeOf + MaxEncodedLen + TypeInfo;
		// + FullCodec + Clone + Eq + PartialEq + Decode + Debug;
	}

	/// Items to be executed, indexed by the block number that they should be executed on.
	#[pallet::storage]
	pub type Agenda<T: Config> =
		StorageMap<_, Twox64Concat, T::RelayBlockNumber, Vec<Option<ScheduledV3Of<T>>>, ValueQuery>;

	/// Lookup from identity to the block number and index of the task.
	#[pallet::storage]
	pub(crate) type Lookup<T: Config> =
		StorageMap<_, Twox64Concat, Vec<u8>, TaskAddress<T::RelayBlockNumber>>;
	...
	pub enum Event<T: Config> {
		/// Scheduled some task.
		Scheduled { when: T::RelayBlockNumber, index: u32 },
		/// Canceled some task.
		Canceled { when: T::RelayBlockNumber, index: u32 },
		/// Dispatched some task.
		Dispatched {
			task: TaskAddress<T::RelayBlockNumber>,
			id: Option<Vec<u8>>,
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn on_initialize(now: T::RelayBlockNumber) -> Weight {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule(
			origin: OriginFor<T>,
			when: T::RelayBlockNumber,
			maybe_periodic: Option<schedule::Period<T::RelayBlockNumber>>,
			priority: schedule::Priority,
		...
		pub fn cancel(origin: OriginFor<T>, when: T::RelayBlockNumber, index: u32) -> DispatchResult {
			T::ScheduleOrigin::ensure_origin(origin.clone())?;
			let origin = <T as Config>::Origin::from(origin);
...
impl<T: Config> Pallet<T> {
	...
	pub fn migrate_v1_to_v3() -> Weight {
		...
		Agenda::<T>::translate::<Vec<Option<ScheduledV1<<T as Config>::Call, T::RelayBlockNumber>>>, _>(
			|_, agenda| {
				Some(
					agenda
						.into_iter()
	...
	pub fn migrate_origin<OldOrigin: Into<T::PalletsOrigin> + codec::Decode>() {
		Agenda::<T>::translate::<
			Vec<Option<Scheduled<CallOrHashOf<T>, T::RelayBlockNumber, OldOrigin, T::AccountId>>>,
			_,
		>(|_, agenda| {
			Some(
				agenda
	...
	fn resolve_time(when: DispatchTime<T::RelayBlockNumber>) -> Result<T::RelayBlockNumber, DispatchError> {
	...
	fn do_schedule(
		when: DispatchTime<T::RelayBlockNumber>,
		maybe_periodic: Option<schedule::Period<T::RelayBlockNumber>>,
		priority: schedule::Priority,
		origin: T::PalletsOrigin,
		call: CallOrHashOf<T>,
	) -> Result<TaskAddress<T::RelayBlockNumber>, DispatchError> {
	...
	fn do_cancel(
		...
		(when, index): TaskAddress<T::RelayBlockNumber>,
	...
	fn do_reschedule(
		(when, index): TaskAddress<T::RelayBlockNumber>,
		new_time: DispatchTime<T::RelayBlockNumber>,
	) -> Result<TaskAddress<T::RelayBlockNumber>, DispatchError> {
	...
	fn do_reschedule_named(
		...
		Lookup::<T>::try_mutate_exists(
			id,
			|lookup| -> Result<TaskAddress<T::RelayBlockNumber>, DispatchError> {
...
impl<T: Config> schedule::v2::Anon<T::RelayBlockNumber, <T as Config>::Call, T::PalletsOrigin>
	for Pallet<T>
{
	type Address = TaskAddress<T::RelayBlockNumber>;
	type Hash = T::Hash;
	...
	fn next_dispatch_time((when, index): Self::Address) -> Result<T::RelayBlockNumber, ()> {
		Agenda::<T>::get(when).get(index as usize).ok_or(()).map(|_| when)
	}
}

impl<T: Config> schedule::v2::Named<T::RelayBlockNumber, <T as Config>::Call, T::PalletsOrigin>
	for Pallet<T>
{
	type Address = TaskAddress<T::RelayBlockNumber>;
	type Hash = T::Hash;
	...
	fn next_dispatch_time(id: Vec<u8>) -> Result<T::RelayBlockNumber, ()> {
		Lookup::<T>::get(id)
